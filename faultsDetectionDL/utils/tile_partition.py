...
def run_partition(site_name, tiles_gdf, rgb_rio_dst, gt_rio_dst, output_folder, 
    ...
    def save_transform(ID, geotransform, rgb_matrix, gt_matrix, tile_type):
        ...
        with rio.open(output_path_templ.format('image',site_name, ID),
                      'w', tiled=True, blockxsize=tile_sizex, blockysize=tile_sizey, driver="GTiff" , width=tile_sizex, height=tile_sizey, count=band_count, crs=rgb_crs, transform=geotransform, nodata=0, dtype=rio.ubyte) as output_rgb_dst:
            output_rgb_dst.write(reshape_as_raster(rgb_matrix).astype(rio.ubyte))
            
        with rio.open(output_path_templ.format('gt',site_name, ID),
                      'w',  tiled=True, blockxsize=tile_sizex, blockysize=tile_sizey, driver="GTiff" , width=tile_sizex, height=tile_sizey, count=1, crs=rgb_crs, transform=geotransform, nodata=0, dtype=rio.ubyte) as output_gt_dst:
            output_gt_dst.write(reshape_as_raster(gt_matrix).astype(rio.ubyte))
